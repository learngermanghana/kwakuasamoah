...
export function BookingForm({ serviceOptions, prefilledServiceName }: BookingFormProps) {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setResultMessage({ kind: "success", text: data.message || "Booking created. Redirecting to secure checkout." });
      ...
      setFormState((previous) => ({ ...previous, customerName: "", customerEmail: "", customerPhone: "", bookingDate: "", bookingTime: "", notes: "", website: "", consent: false, agreement: false }));
      setErrors({});
    } catch {
  ...
  return <form onSubmit={onSubmit} className="mt-8 space-y-4" noValidate>{/* trimmed for brevity */}
    ...
    <button type="submit" className="rounded-2xl bg-emerald-600 px-6 py-3 font-semibold text-white disabled:opacity-60" disabled={isSubmitting}>{isSubmitting ? "Submitting..." : "Continue to secure checkout"}</button>
  </form>;
}
